...
#else
#warning ARM BUILD LE TO LE ONLY

#include <sys/sendfile.h>
int _sendFile;

void setSendFile(int enable) {
	_sendFile = enable;
}
...
int *dims, char *bytes)
{
  ...
  SocketSend(sock, (char*)m, sizeof(MsgHdr), 0);
  if (_sendFile){
	  sendfile(sock, (int)bytes, 0, nbytes);
  }else{
	  SocketSend(sock, bytes, nbytes, 0);
  }
